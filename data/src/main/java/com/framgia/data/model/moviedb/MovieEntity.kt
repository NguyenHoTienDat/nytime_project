...
import com.framgia.domain.model.moviedb.Movie
...
class MovieEntityMapper @Inject constructor(
    ...
    override fun mapToEntity(model: Movie): MovieEntity =
        MovieEntity(
            ...
            model.overview,
            model.popularity,
            model.productionCompanyEntities?.map { productionCompanyEntityMapper.mapToEntity(it) },
            model.productionCountryEntities?.map { productionCountryEntityMapper.mapToEntity(it) },
            model.releaseDate,
            model.revenue,
            model.runtime,
            model.spokenLanguageEntities?.map { spokenLanguageEntityMapper.mapToEntity(it) },
            model.status,
            model.tagLine,
            model.title,
            model.video,
